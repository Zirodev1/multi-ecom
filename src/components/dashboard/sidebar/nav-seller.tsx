...
export default function SidebarNavSeller({menuLinks}:{menuLinks:DashboardSidebarMenuInterface[]}) {
  ...
  const storeUrlStart = pathname.split("/shops/")[1];
                ...
                return (
                  <CommandItem
                    ...
                    className={cn("w-full h-12 cursor-pointer mt-1", {
                      "bg-accent text-accent-foreground" : link.link === "" ? pathname === `/dashboard/seller/shops/${activeStore}` : `/dashboard/seller/shops/${activeStore}/${link.link}` === pathname,
                    })}
                  >
                    <Link href={`/dashboard/seller/shops/${activeStore}/${link.link}`} className="flex items-center gap-2 hover:bg-transparent rounded-md transition-all w-full" > {icon} <span>{link.label}</span> </Link>
                  </CommandItem>
                )
            })}
          </CommandGroup>
